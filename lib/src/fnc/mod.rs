...
/// Creates a (possibly async) dispatcher fn.
/// 
/// Each SurQL function is specified by its name (a string literal) followed by its path. The path
...
macro_rules! dispatch {
	($($color:tt)? fn $dispatcher:ident, $($function_name:literal => $(($wrapper:tt))* $($function_path:ident)::+ $(($ctx_arg:expr))* $(.$await:tt)*,)+) => {
		pub $($color)? fn $dispatcher(ctx: &Context<'_>, name: &str, args: Vec<Value>) -> Result<Value, Error> {
			match name {
				$($function_name => {
		...
		// TODO: Could we use this outside of testing to expedite parsing?
		#[cfg(test)]
		mod $dispatcher {
			/// For testing purposes, list all the functions.
			pub(super) const FUNCTION_NAMES: &[&'static str] = &[
				$($function_name),*
			];
		}
	};
...
#[cfg(test)]
mod tests {
    use super::{synchronous, asynchronous};

	#[test]
	fn all_functions_parse() {
		for &name in synchronous::FUNCTION_NAMES.iter().chain(asynchronous::FUNCTION_NAMES) {
			assert!(crate::sql::function::function(name).is_ok(), "could not parse function {name}");
		}
	}

	#[test]
	fn all_functions_exported_to_js() {
		for &name in synchronous::FUNCTION_NAMES.iter().chain(asynchronous::FUNCTION_NAMES) {
			// TODO.
		}
	}
